...
export default function FreeShippingBarPage() {
  const [active, setActive] = useState(false)
  const [loading, setLoading] = useState(true)

  async function loadWidget() {
    try {
      const res = await fetch("/api/widgets/free-shipping-bar")
  ...
  return (
    <main style={styles.main}>
      ...
      <h1 style={styles.title}>Free Shipping Bar</h1>

      <div style={styles.card}>
        <p style={styles.muted}>Barre de livraison offerte pour encourager l’ajout au panier.</p>
        ...
        >
          {loading ? "Chargement..." : active ? "Désactiver Free Shipping Bar" : "Activer Free Shipping Bar"}
        </button>
      </div>
    </main>
  )
